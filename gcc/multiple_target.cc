...
/* When NODE is a target clone, consider all callees and redirect
   to a clone with the most priority callee that compatible with the
   caller.  If TARGET_VERSION_COMPATIBLE is not defined, we only
   redirect to the clone with the same target attribute.  That
   prevents multiple multi-versioning dispatches and a call-chain
   can be optimized.  */
		  ...
		  if (targetm.compare_version_priority
		      && targetm.version_compatible)
		    {
			  ...
			  && (best_callee == NULL
				 ...
				 (best_callee->decl, callee->decl) < 0))
			best_callee = callee;
		    }
		  else
		    {
		      /* For targets does not defined TARGET_VERSION_COMPATIBLE,
			 we need to check if the target attributes are
			 equal.  */
