...
riscv_compare_version_priority (tree decl1, tree decl2)
{
  ...
  return 0; // TODO: return compare_feature_masks
}
...
add_condition_to_bb (tree function_decl, tree version_decl,
		     unsigned long long feature_mask, // TODO: RV format
			    ...
			    basic_block *empty_bb)
{
  ...
  for (tree version_decl : *fndecls)
    {
      ...
      if (version_attr == NULL) {
        fprintf(stderr, "version_attr is NULL\n");
      }
      else {
      ...
      function_versions [actual_versions].feature_mask = actual_versions;
      actual_versions++;
    }
  ...
  auto compare_feature_version_info = [](const void *p1, const void *p2) {
    ...
    if (v1.feature_mask == v2.feature_mask)
      return 0;
    return v1.feature_mask < v2.feature_mask ? 1 : -1;
  };
...
riscv_mangle_decl_assembler_name (tree decl, tree id)
{
  ...
  if (TREE_CODE (decl) == FUNCTION_DECL
      && DECL_FUNCTION_VERSIONED (decl))
    {
      ...
      if (target_opts->x_riscv_zb_subext != 0) {
        name += ".zbb";
      }
      else if (target_opts->x_target_flags & MASK_FULL_V) {
        name += ".v";
      }
      else {
        name += ".default";
      }
      fprintf(stderr, "riscv_mangle_decl_assembler_name: %s\n", name.c_str());
      return get_identifier (name.c_str ());
    }
